    ...
    ipcMain.handle("export:csv", async (event) => {
        ...
        try {
            ...
            if (voluntarios.length === 0) {
                return { success: false, message: "Nenhum voluntário encontrado para exportar." };
            }
            ...
            for (const voluntario of voluntarios) {
                const avaliacoes: Avaliacao[] = await avaliacaoService.listByVoluntario(voluntario.id!);
            ...
            return { success: false, message: 'Ocorreu um erro ao exportar os dados.' };
        }
    });
}